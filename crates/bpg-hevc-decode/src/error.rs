//! Error types for the vendored HEVC decoder.
//!
//! Vendored from `heic-decoder-rs/src/error.rs` (the `HevcError` portion only).
//! The HEIF-container error type (`HeicError`) and the `enough`/`whereat`
//! location-tracking machinery are intentionally left behind — BPG decode only
//! needs the HEVC bitstream errors.
